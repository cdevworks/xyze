...
import "../token/KRAPTokenInterface.sol";


contract KRAPRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 krapsSold, uint256 krapsFromReserves, uint256 krapsToReserves);
    ...
    /// @notice KRAP token address
    address public krapAddress;
    ...
    /// @notice pair for reserveToken <> KRAP
    ...
    /// @notice Whether or not this token is first in uniswap KRAP<>Reserve pair
    bool public isToken0;

    constructor(
        address krapAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(krapAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == krapAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap KRAP<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        KRAPTokenInterface krap = KRAPTokenInterface(krapAddress);

        if (positive) {
            require(krap.krapsScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < krap.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = krap.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = krap.rebase(epoch, indexDelta, positive);
        assert(krap.krapsScalingFactor() <= krap.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            krap.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.krapsToUni) {
                // if the amount from reserves > krapsToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                krap.mint(uniswap_pair, uniVars.krapsToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            krap.mint(uniswap_pair, uniVars.krapsToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            krap.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.krapsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.krapsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess krap in the reserve
        uint256 excess = krap.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          krapsToUni: tokens_to_max_slippage, // how many kraps uniswap needs
          amountFromReserves: excess, // how much of krapsToUni comes from reserves
          mintToReserves: 0 // how much kraps protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.krapsToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire krap reserves, and minting part of total
