...
// Modified to work in the KRAP system

// all votes work on underlying _krapBalances[address], not balanceOf(address)
...
//
// Changes made by KRAP after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since KRAP supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match KRAP as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "KRAP Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(krap.initSupply(), 4), 100); } // 4% of KRAP

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(krap.initSupply(), 100); } // 1% of KRAP
    ...
    constructor(address timelock_, address krap_) public {
        timelock = TimelockInterface(timelock_);
        krap = KRAPInterface(krap_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(krap.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || krap.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = krap.getPriorVotes(voter, proposal.startBlock);
